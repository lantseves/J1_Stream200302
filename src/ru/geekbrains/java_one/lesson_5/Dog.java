...
public class Dog extends Animal {
    ...
    public Dog() {
        this.maxDistanceRun = Math.random() * 500d ;
        this.maxDistanceSwim = Math.random() * 10d ;
        this.maxHeightJump =  Math.random() * 0.5d ;
    }

    @Override
    public String run(int distance) {
        return distance <= maxDistanceRun ?
                "Пёсик пробежал " + distance + " м." :
                "Пёсик пробежал " + maxDistanceRun + " м." ;
    }

    @Override
    public String swim(int distance) {
        return distance <= maxDistanceSwim ?
                "Пёсик проплыл " + distance + " м." :
                "Пёсик проплыл " + maxDistanceSwim + " м." ;
    }

    @Override
    public String jump(int height) {
        return height <= maxHeightJump ?
                "Пёсик прыгнул на " + height + " м." :
                "Пёсик прыгнул на " + maxHeightJump + " м." ;
    }
}
